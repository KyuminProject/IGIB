...
.blueTable>table>tbody>tr.highlight-parent {
   border: 1px solid darkgrey;
...
.redTable>table>tbody>tr.highlight-parent {
   border: 1px solid darkgrey;
...
<body>
   ...
   <div class="masthead">
      ...
      <div class="masthead-content text-white">
         <div class="container-fluid px-4 px-lg-0" id="Main-Container" style="overflow: auto; height: 340px">
           <div id="title" style="width: 100%; text-align: center;">
            <h1 class="fst-italic lh-1 mb-4;" style="color: whitesmoke;">Winning Rate by Friend
            </h1>
            ...
            <div id="questionMarkBox" style="display: none;z-index: 10; background-color: rgba(0, 0, 0, 0.7);">
               <div id="explainBox">
                  <span>내친구와 랭크 승률</span>
               </div>
            </div>
           </div>
            <!-- 검색창 -->
         <div class="container" id="search">
            <form id="contactForm" data-sb-form-api-token="API_TOKEN" class="row g-3" id="myForm">
               <div id="selectServer">
                  <input type="text" class="form-control" id="tag" name="tag" placeholder="tag (#제외)">
               </div>
               <div id="searchNickname" style="width:80%;">
               ...
               <div id="selectServer">
                  <input type="text" class="form-control" id="friendTag" name="friendTag" placeholder="tag (#제외)">
               </div>
               <div id="searchNickname" style="width:80%;">
                  <input class="form-control" id="friend" name="friend" type="text" placeholder="친구 소환사 이름" aria-label="Enter Nickname" data-sb-validations="required,email" />
               </div>
               <div id="searchBtn" style="width:100%; text-align: -webkit-right;">
                  <button type="button" class="btn btn-primary" id="myButton" style="width:30%;  padding: 0.375rem 0.75rem;
    justify-content: center;">Find</button>
               </div>
            </form>
         </div>
         <!-- 검색창 끝 -->
         ...
         <div id="resultBox">
          <!-- 퍼센트바 html -->
             <div id="percentBar" class="progress-bar_result" data-percent="" style="display: none; margin-top: 1%;">
                <div class="fill_result"></div>
                <span class="percent-label_result"></span>
           </div>
         </div>
      </div>
   </div>
   ...
   $(document).ready(function() {
      $("#myButton").click(function() {
         const fields = {
              ...
              friendTag: document.getElementById("friendTag"),
            };
            ...
            if (emptyFields.length > 0) {
              for (const field in emptyFields) {
                message += fields[emptyFields[field]].placeholder + ", ";
              }
              message = message + "입력 해주세요";
              alert(message);
              fields[emptyFields[0]].focus(); // 첫 번째 비어 있는 필드로 포커스 이동
       ...
       $.ajax({
          type: "POST",
            url: "http://kyu233.iptime.org:200/FriendMatch",
            data: data,
           success: function(response) {
               console.log(response);
               const container = $("#resultSerach");
               //container.empty(); 
               ...
               for (const game of response) {
                   ...
                   if ((game.teamId == 200 && game.win == true) || (game.teamId == 100 && game.win == false)) {
                       red_win = '승리';
                       blue_win = '패배';
                   } else { // else if 제거하고 조건 없이 else 사용
                       red_win = '패배';
                       blue_win = '승리';
                   }
                   ...
                   blueTableHtml +='<thead>'
                   blueTableHtml += '<tr>'
                   blueTableHtml += '<th style="width: 24%; text-align: left;">'+red_win+'(레드팀)</th>'
                   blueTableHtml += '<th style="width: 13%;">KDA</th>'
                   blueTableHtml += '<th style="width: 20%;">피해량</th>'
                   blueTableHtml += '<th style="width: 6%;">와드</th>'
                   blueTableHtml += '<th style="width: 8%;">CS</th>'
                   blueTableHtml += '<th style="width: 28%">아이템</th>'
                   blueTableHtml += '</tr>'
                   blueTableHtml += '</thead>'
                   ...
                   for (let i = 0; i < 5; i++) {
                       ...
                       blueTableHtml += '</td>';
                       blueTableHtml += '<td>';
                       blueTableHtml += '<div class="table-ward">';
                       blueTableHtml += '<span>'+player.visionWardsBoughtInGame+'</span>';
                       blueTableHtml += '</div>';
                       blueTableHtml += '</td>';
                       blueTableHtml += '<td>';
                       ...
                       blueTableHtml += '</td>';
                       blueTableHtml += '<td>';
                       blueTableHtml += '<div class="table-item">';
                       
                       for(const player_item of player.item_lst){
                           blueTableHtml += '<div class="table-inner-item">';
                           blueTableHtml += '<img alt="" src="/assets/img/item/'+player_item+'.png">'; // 아이템 1 이미지
                           blueTableHtml += '</div>';                        
                       }
                   ...
                   redTableHtml +='<thead>'
                   redTableHtml += '<tr>'
                   redTableHtml += '<th style="width: 24%; text-align: left;">'+blue_win+'(블루팀)</th>'
                   redTableHtml += '<th style="width: 13%;">KDA</th>'
                   redTableHtml += '<th style="width: 20%;">피해량</th>'
                   redTableHtml += '<th style="width: 6%;">와드</th>'
                   redTableHtml += '<th style="width: 8%;">CS</th>'
                   redTableHtml += '<th style="width: 28%">아이템</th>'
                   redTableHtml += '</tr>'
                   redTableHtml += '</thead>'
                   ...
                   for (let i = 5; i < 10; i++) {
                       ...
                       redTableHtml += '</td>';
                       redTableHtml += '<td>';
                       redTableHtml += '<div class="table-ward">';
                       redTableHtml += '<span>'+player.visionWardsBoughtInGame+'</span>';
                       redTableHtml += '</div>';
                       redTableHtml += '</td>';
                       redTableHtml += '<td>';
                       ...
                       redTableHtml += '</td>';
                       redTableHtml += '<td>';
                       redTableHtml += '<div class="table-item">';
                       
                       for(const player_item of player.item_lst){
                           redTableHtml += '<div class="table-inner-item">';
                           redTableHtml += '<img alt="" src="/assets/img/item/'+player_item+'.png">'; // 아이템 1 이미지
                           redTableHtml += '</div>';                        
                       }
               ...
               const winrate = $("#winrate");
                winrate.empty()
                const win = $('.win-background').length;
                const lose = $('.lose-background').length;
                let win_html =""
                if ((win+lose)==0){
                   alert("전적이 없습니다 다시 검색해주세요")
                   window.location.replace("/")
                }else {
                   const $percentBar = $("#percentBar");
                   winPercent = Math.round((win/(win+lose)*100000)/1000);
                   console.log(winPercent);
                   $percentBar.attr('data-percent', winPercent);
                   var progressBars = document.querySelectorAll(".progress-bar");

                   $("#percentBar").css("display", "");
                }
                winrate.append(win_html);
           },
           beforeSend:function(){
               /*(이미지 보여주기 처리)*/
               $('.wrap-loading').removeClass('display-none');
               $('#winrate').addClass('display-none');
           },

           complete:function(){
               /* (이미지 감추기 처리) */
               $('.wrap-loading').addClass('display-none');
               $('#winrate').removeClass('display-none');
               $(".masthead-content").css("margin-top", "0");
               
               var progressBars = $("#progress-bar_result");
               var fill = $(".fill_result");
               var percentLabel = $(".percent-label_result");
               
                fill.css("width", winPercent + "%");  // 채워진 부분 너비 설정
              percentLabel.text("승률 :    "+winPercent + "%");
           },
       
           error: function(xhr, status, error) {
              console.error("AJAX 요청 실패:", error);
               alert("검색되는 소환사 없습니다, 소환사명을 다시 확인해주세요");
    ...
    <script type="text/javascript">
         ...
         document.addEventListener("DOMContentLoaded", function() {
              /*  var progressBars = $("#progress-bar_result");
               var fill = $(".fill_result");
               var percentLabel = $(".percent-label_result");
               var percent = progressBars.attr("data-percent")

                fill.css("width", percent + "%");  // 채워진 부분 너비 설정
              percentLabel.text(percent + "%");  // 퍼센트 레이블 텍스트 업데이트 */
         });
      </script>